...
def VC_Manage_Project  = 'Project_vcast_pipeline/working_dir/Project.vcm'
def VC_EnvSetup        = """
...
"""
def VC_Build_Preamble  = ""
def VC_EnvTeardown     = ''''''
def scmStep () { git branch: 'main', url: 'https://github.com/TimSVector/vce_plugin_local_tests.git' }
def VC_usingSCM = true
def VC_postScmStepsCmds = "call %WORKSPACE%/Project_vcast_pipeline/cleanup_and_build.bat"
def VC_sharedArtifactDirectory = ''''''
def VC_Agent_Label = 'Host_Test_Node_Win'
def VC_waitTime = '30'
def VC_waitLoops = '1'
def VC_maxParallel = 0
                ...
                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                    ...
                    script {
                        ...
                        if (VC_useCoveragePlugin) {
                            ...
                            buildLogText += runCommands("""_VECTORCAST_DIR/vpython  "${env.WORKSPACE}"/vc_scripts/vcast_exec.py ${VC_Manage_Project}  --utfull --tcmr --fullstatus --aggregate --metrics""", VC_EnvSetup, VC_useCILicense)
                        }
