'''
This file contains functions that will be invoked by VectorCAST to
provide custom build and execute commands based on the name of a
VectorCAST/Cover environment in a VectorCAST Project.
'''
...
class SystemTestsConfiguration(object):
    ...
    def commandToRunATest(self, test_case):
        ...
        '''
        command = self.nameOfTestExecutable + ' ' + test_case.get_name()
        # if not an absolute path, make it relative to the current directory
        if not os.path.isabs(self.nameOfTestExecutable):
...
class DefaultSystemTests(SystemTests):
    ...
    def run_test_case(self, test_case):
        ...
        with system.cd(self.configuration.locationWhereWeRunTests):
            # If you have your instrumented application configured to use file output
            # The coverage data will be in the TESTINSS.DAT file after the test is run
            if os.path.isfile(self._get_coverage_data_file_path()):
                os.remove(self._get_coverage_data_file_path())

            # Run the correct test, based on the TestCase type
            if test_case.is_manual():
                commandStatus = manualTest(
                    test_case.get_name(),
            ...
            # The call to save_test_case_result() stores the coverage data and pass / fail status for the test
